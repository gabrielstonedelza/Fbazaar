...
def alert_order(sender, created, instance, **kwargs):
    ...
    if created:
        ...
        # send_my_mail(f"New Order", settings.EMAIL_HOST_USER, settings.EMAIL_HOST_USER, {"item_name": instance.cart.ordered_item.name, "quantity": instance.quantity, "username":instance.user.username},
...
def alert_order_cleared_for_pickup(sender, created, instance, **kwargs):
    ...
    message = f"Your order has been cleared for processing and pickup"
    admin_user = User.objects.get(id=1)
    ...
    if created:
        ...
        #              {"item_name": instance.order_item.cart.ordered_item.name,
...
def alert_order_dropped_off(sender, created, instance, **kwargs):
    ...
    message = f"Order has been delivered to the customers location"
    admin_user = User.objects.get(id=1)
    user_ordering = User.objects.get(username=instance.order.user.username)
